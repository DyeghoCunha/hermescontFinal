...
const CardParallax = ({ i,title, description, src, color}: ICardParallax) => {
  ...
  return (
    <Box
      ...
      >
        <HStack color="white" position="relative" w="100%" mt="200px" pl="100px" >
          <ClientImg h={{ base: "30px", md: "120px" }} m={{base:"8px",md:"0"}} src={Triangulo.src} />
          <Box position="absolute" top={{base:"-1px",md:"2.5vh",lg:"20px"}} left={{ base: 10, md: "230px" }} >
            <HermesCharacter colorProp={"white"} fontSize={{ base: "20px", xl: "45px",xxl:"50px" }} paragraph={title} />
          </Box>
        </HStack>

        <Box px={{base:"10%",xl:"20%",xxl:"25%"}} mt="100px" color="white">
         
            <Text fontSize={{ base: "10px", md: "30px" }}  display={i === 2 ? "none" : "block"}>
              <Box as="span" fontSize={{base:"20px",md:"48px"}} fontFamily="Title" >
                {description.charAt(0)}
              </Box>
              {description.slice(1)}
            </Text>
            <VStack color="white" display={i === 2 ? "block" : "none"} fontSize={{ base: "10px", md: "30px" }} mt={{base:"5px",md:"50px",xxl:"55px"}} >
